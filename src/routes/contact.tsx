...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Зв'яжіться зі Spice Market: пошта, телефон, шоурум у Києві." },
      { property: "og:title", content: "Контакти — Spice Market" },
      { property: "og:description", content: "Зв'яжіться зі Spice Market: пошта, телефон, шоурум у Києві." },
    ...
    <div className="container mx-auto grid gap-12 px-4 py-16 lg:grid-cols-2">
      <div>
        ...
        <ul className="mt-10 space-y-5">
          ...
          <Item icon={<MapPin className="h-5 w-5" />} title="Шоурум" value="Київ, вул. Хрещатик 22, пн–сб" />
        </ul>
      </div>
      <form
        onSubmit={(e) => {
      ...
      >
        ...
        <div>
          <label className="text-xs uppercase tracking-wider text-muted-foreground">Повідомлення</label>
          <textarea required rows={5} className="mt-1 w-full rounded-sm border border-border bg-background px-3 py-2 text-sm focus:border-accent focus:outline-none" />
        </div>
...
function Item({ icon, title, value }: { icon: React.ReactNode; title: string; value: string }) {
  return (
    <li className="flex items-start gap-4">
      <span className="grid h-10 w-10 place-items-center rounded-sm bg-secondary text-accent">{icon}</span>
      <div>
